...
## Fix round 8 — cancellation and cleanup ownership

- Completion now honors request cancellation before the durable phase, calls `CommitCompletionAsync` with `CancellationToken.None` once that phase starts, and reconciles every commit exception (including cancellation). A durable exact match returns the completed opaque DTO and preserves rendition objects; an absent outcome follows normal retry compensation; an indeterminate reconciliation remains intentionally non-destructive.
- Cleanup now reloads and claims the exact candidate under a row lock before any storage delete. The claim records immutable staging/attempt keys and a cleanup claim token; success clears only that claim, while delete failure releases the claim but retains the durable candidate for a future worker. Cleanup exceptions log only a fixed event plus exception type, never SDK exception text.
- Retryable attempt cleanup reports synchronous delete success. When it fails, releasing a processing lease durably copies that lease to `CleanupProcessingLeaseId`; ordinary completion claims are blocked until token-scoped final cleanup has run. Expired tickets terminalize into `Expired`, so a successful cleanup is excluded from future scans.
