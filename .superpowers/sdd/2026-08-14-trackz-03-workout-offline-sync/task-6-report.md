...
- A persisted `IWeightUnitPreference` is composed through MAUI Preferences. The native selector updates that preference; entry, Last/Today rows, and existing Exercise Picker Last/PR cards refresh consistently in localized kg or lb. Picker kg values preserve the supported three-decimal precision and pounds use stable two-decimal display precision, while ViewModels, cache DTOs, and SQLite remain canonical kg.
...
20. Round 3 picker-unit coverage first failed because `ExercisePickerViewModel` had no unit-preference composition. The real picker ViewModel now proves exact `70.125 kg`/`25.125 kg`, live conversion to `154.60 lb`/`55.39 lb`, localized Thai pound labels, persisted recomposition, weighted/assisted shapes, and unchanged canonical cache values. Real MAUI provider activation proves the composed picker consumes the shared singleton preference.
21. Deterministic feedback-phase tests first failed because no generation-bound phase session or page pulse driver existed. Reset now atomically wins or loses each actual main-thread haptic/pulse start under the account-session transition lock. Tests cancel in the former dispatch/start check-use gap and observe zero haptic/pulse starts; a pulse already started is canceled promptly. The session boundary is not held for the animation, and cancellation remains best effort after durable save.

## Verification

- `dotnet test tests/TrackZ.Mobile.Tests/TrackZ.Mobile.Tests.csproj --filter Workout --no-restore -m:1 -nr:false` — PASS, 66/66.
- `dotnet test tests/TrackZ.Mobile.Tests/TrackZ.Mobile.Tests.csproj --filter "FullyQualifiedName~SetLoggerViewModelTests|FullyQualifiedName~MauiSetSavedFeedbackTests|FullyQualifiedName~ExercisePickerViewModelTests|FullyQualifiedName~MauiCompositionTests" --no-restore -m:1 -nr:false` — PASS, 75/75.
- `dotnet test tests/TrackZ.Mobile.Tests/TrackZ.Mobile.Tests.csproj --filter "FullyQualifiedName~ExercisePickerViewModelTests|FullyQualifiedName~ExerciseCache|FullyQualifiedName~CustomExerciseViewModelTests|FullyQualifiedName~LocalExerciseImageImporterSecurityTests" --no-restore -m:1 -nr:false` — PASS, 75/75 moved picker/cache regressions.
- `dotnet test tests/TrackZ.Mobile.Tests/TrackZ.Mobile.Tests.csproj --no-restore -m:1 -nr:false` — PASS, 212/212.
