...
public static class GamificationResources
{
    ...
    public static GamificationTextSet Thai { get; } = new(
        "สรุปการออกกำลังกาย", "ผลการฝึก", "โปรไฟล์", "คุณ", "ความก้าวหน้ารอยืนยันจากเซิร์ฟเวอร์",
...
public sealed class WorkoutSummaryViewModel : GamificationViewModelBase
{
    ...
    public ProgressReveal? Reveal
    {
        ...
        private set
        {
            ...
            OnPropertyChanged(nameof(IsProgressRevealConfirmed));
            OnPropertyChanged(nameof(IsProgressRevealPending));
            OnPropertyChanged(nameof(HasProgressReveal));
        }
    }
...
public sealed class ProfileViewModel : GamificationViewModelBase
{
    ...
    private void Apply(ProgressSnapshot snapshot)
    {
        ...
        Badges.Clear();
        foreach (var badge in profile.Badges) Badges.Add(EarnedBadgePresentation.From(badge, CultureInfo.CurrentUICulture));
    }
}
