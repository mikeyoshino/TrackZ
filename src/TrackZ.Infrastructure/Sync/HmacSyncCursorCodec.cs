...
public sealed class HmacSyncCursorCodec(IOptions<JwtOptions> options) : ISyncCursorCodec
{
    ...
    private static byte[] Decode64(string value)
    {
        ...
        var decoded = Convert.FromBase64String(base64.PadRight(base64.Length + (4 - base64.Length % 4) % 4, '='));
        if (!string.Equals(value, Encode64(decoded), StringComparison.Ordinal)) throw new FormatException();
        return decoded;
    }
