...
public sealed class ImageUploadTicketConfiguration : IEntityTypeConfiguration<ImageUploadTicket>
{
 public void Configure(EntityTypeBuilder<ImageUploadTicket> builder)
 {
  ...
  {
   table.HasCheckConstraint("CK_image_upload_tickets_state", "\"State\" IN (1, 2, 3, 4, 5)");
   table.HasCheckConstraint("CK_image_upload_tickets_processing_lease", "(\"State\" = 3 AND \"ProcessingStartedAt\" IS NOT NULL AND \"LeaseExpiresAt\" IS NOT NULL AND \"ProcessingLeaseId\" IS NOT NULL) OR (\"State\" <> 3 AND \"ProcessingStartedAt\" IS NULL AND \"LeaseExpiresAt\" IS NULL AND \"ProcessingLeaseId\" IS NULL)");
  });
